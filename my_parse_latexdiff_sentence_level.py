...
def split_and_align(samples: List[Dict], outfile_path: str):
    ...
    def _split_and_align(sample: Dict) -> List[Dict]:
        ...
        for action in edit_actions:
            sent_level_action = _handle_each_action(
                before,
                before_sents,
                action,
                char_to_sent,
...
if __name__ == "__main__":
    ...
    def process_dir(file_path: str) -> None:
        print(f"Processing file: {file_path}")

        with open(file_path, "r") as f:
            samples = json.load(f)

        len_edit_actions = sum(len(sample["edit_actions"]) for sample in samples)

        outfile_path = os.path.join(base_dir, "sent_" + os.path.basename(file_path))
        all_sent_level_actions = split_and_align(samples, outfile_path)

        print(f"Processed {len(samples)} samples with {len_edit_actions} edit actions.")

    my_config = deepcopy(en_config)
    my_config["mincut"] = 10
    splitter = SentSplit("en", **my_config)

    base_dir = "./data/filtered_revisions"
    doc_level_files = ["doc-level_wikipedia.json"]
    for doc_level_file in doc_level_files:
        doc_level_path = os.path.join(base_dir, doc_level_file)
        process_dir(doc_level_path)
