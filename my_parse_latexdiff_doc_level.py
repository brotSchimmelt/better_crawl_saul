...
class LatexdiffParser:
    ...
    def parse_and_save(self) -> int:
        ...
        parsed_samples, skipped_files = [], []
        for diff_f_p in tqdm(diff_file_paths, desc="Parsing LaTeX diffs"):
    ...
    def parse_abstract(self, abstract: str) -> Tuple[str, str, Dict]:
        ...
        def _parse_abstract(matched: re.Match, action_type: str) -> str:
            ...
            action = {"type": action_type, "before": before, "after": after}
            if action_type == "R":
            ...
            if char == self.anchor_symbol:
                ...
                if action_type in ["R", "D"]:
                    ...
                    if before_part != abstract[cursor + 3 : cursor + 3 + len(before_part)]:
                        print(
                            f'ERROR: "{before_part}" != "{abstract[cursor + 3:cursor + 3 + len(before_part)]}"'
                        )
...
if __name__ == "__main__":
    ...
    def process_dir(base_dir: str) -> None:
        latexdiff_dir_names = ["wikipedia"]  # TODO: ['arxiv', 'news', 'wiki']
        len_samples = {}
        for dir_name in latexdiff_dir_names:
            dir_path = os.path.join(base_dir, "tmp_" + dir_name)
    ...
    # process 4K
    base_dir_4k = "./data/filtered_revisions/"
